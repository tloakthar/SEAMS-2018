...
# logistic regression classifiers as the number
# of attack instances increase

# The script is used to generate plots in
# Fig. 9 of the SEAMS submission.
...
for (n_users in c(6,9)) {
  ...
  for (k in c(1:length(sizes))) {
    ...
    for (i in 2:length(rindex)) {#postive instances
      for (j in 1:10) {
        ...
        train.data<-train.data[,apply(train.data, 2, var, na.rm=TRUE) != 0] # remove columns with constant values only, so that there is no independent variable in the dataframe
        test.data <- requests_data[,colnames(train.data)] # make sure that the test data has the same variable as training data
        ...
        if (dim(table(round(classification.result.a),test.lab))[1] == 2){
          attack_positive[i,j] <-
            table(round(classification.result.a),test.lab)[2,2] / length(attack_cases)
        } else {
          attack_positive[i,j] <- 0
        } 
        attack_negative[i,j] <-
    ...
    out_fname <- paste("figs/logreg-perf-inc",n_users,sizes[k],sep = "-")
